    ...
    <transition name="fade">
        <div class="col-md-6" v-if="product.title">
            ...
            <div class="d-flex justify-content-between align-items-end">
                <span class="h6"><del>原價: {{product.origin_price | currency}}</del></span>
            ...
            <div class="row mt-3 d-flex justify-content-between">
                ...
                <div class="col-md-3 mt-3 mt-md-0 d-flex align-items-end justify-content-sm-start justify-content-md-end">
                    <button class="btn btn-primary" @click.prevent="addToCart(product.id,product.num)">加入購物車</button>
                </div>
            </div>
        </div>
    </transition>
...
export default {
    ...
    methods:{
        getSingle(productId){
            ...
            const api =`${process.env.VUE_APP_API}/api/${process.env.VUE_APP_CUSTOM}/product/${productId}`;
            vm.isLoading = true;
        ...
        addToCart(id,qty=1){
            ...
            let data ={ 'product_id': id, 'qty': qty }

            vm.$http.post(api,{data: data }).
            then(response =>{
                console.log(response);
